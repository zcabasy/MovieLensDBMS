...
- **Flask defaults that are changed**:
    ...
    - `ProxyFix` middleware is enabled (check `movielens/app.py`)
...
```sh
git clone https://github.com/nickjj/docker-flask-example movielens
cd movielens
...
The app is named `movielens` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "movielens" and update a few Docker related resources.
